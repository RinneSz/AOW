...
class NoDataRankDistillationTrainer(metaclass=ABCMeta):
    def __init__(self, args, model_code, model, bb_model, surrogate_test_loader, bb_test_loader, export_root, loss='ranking', tau=1., margin_topk=0.5, margin_neg=0.5):
        self.args = args
    ...
    def calculate_metrics(self, batch, val=True, similarity=False):
        ...
        if isinstance(self.model, BERT):
            ...
            if self.args.dataset_code == 'beauty' and val:
            ...
            with torch.no_grad():
                if isinstance(self.bb_model, BERT):
                    ...
                    for j in range(self.max_len - 1):
                        ...
                        _, sorted_items = to_rank(labels[:, 1:-1], k)
                    ...
                    for j in range(self.max_len - 1):
                        ...
                        # sorted_items = sorted_items[:, :k] + 1
                        _, sorted_items = to_rank(labels[:, 1:], k)
    ...
    def test(self, test_watermark=False):
        try:
        ...
        with torch.no_grad():
            ...
            if test_watermark == False:
                with open(os.path.join(self.export_root, 'logs', 'test_metrics.json'), 'w') as f:
                    json.dump(average_metrics, f, indent=4)
            else:
                with open(os.path.join(self.export_root, 'logs', 'test_metrics_on_watermarks.json'), 'w') as f:
                    json.dump(average_metrics, f, indent=4)
